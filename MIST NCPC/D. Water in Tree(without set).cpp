...
void solve(){
    ...
    while(q--){
        ...
        if(type==1){
            ...
            int from=vec[first];
            if(from==-1) continue;
            int ind=orderIndex[from];
            while(ind<=orderIndex[u] && x--){
                if(color[order[ind]]){
                    x++,ind++,vec[first]=order[ind];continue;
                }
                color[order[ind]]=1;
                if(ind==sz(order)-1){
                    vec[first]=-1;
