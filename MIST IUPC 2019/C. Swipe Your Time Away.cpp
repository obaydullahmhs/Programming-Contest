...
void solve(){
    int n=nxt(),m=nxt();
    vector<vector<int>> v(n+2,vector<int>(m+2)),row(n,vector<int>(m+2,0)),coloumn(n,vector<int>(m,0));
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            v[i][j]=nxt();
        }
    ...
    for(int i=0;i<m;i++){
        ...
        for(int j=k;j<n;j++) coloumn[j][i]=cnt;
    }
    int ans=0;
    for(int i=1;i<n-1;i++){
        for(int j=1;j<m-1;j++){
            if(v[i][j]==v[i-1][j] && v[i][j]==v[i+1][j] && v[i][j]==v[i][j-1] && v[i][j]==v[i][j+1]){
                ans=max(ans,coloumn[i][j]+row[i][j]-1);
            }
        }
    }
    printf("Case %d: ", ++cs);
