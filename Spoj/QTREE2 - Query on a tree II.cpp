...
public:
    ...
    int kth(int u, int v,int k){
        int l=findLCA(u,v);
        int dif=depth[u]-depth[l];
        dif++;
        if(dif>=k) return levelUp(k-1,u);
        else{
            dif=k-dif;
            int x=depth[v]-depth[l];
            dif=x-dif;
            return levelUp(dif,v);
        }
    }
...
void solve(){
    ...
    while(true){
        ...
        int a,b,c;
        cin>>a>>b;
        if(s=="DIST") cout<<lca.Durotto(a,b)<<endl;
        if(s=="KTH"){
            cin>>c;
            cout<<lca.kth(a,b,c)<<endl;
        }
    }
